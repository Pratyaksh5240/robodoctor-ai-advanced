    ...
    <div className="min-h-screen bg-[var(--background)] px-6 py-8 text-[var(--foreground)] md:p-16">
      ...
      {result && (
        ...
        >
          ...
          {/* 4. Personalized Recommendations (Driven by SHAP Feature Attribution & Clinical Triage) */}
          <div className="border-t border-[color:var(--border)] pt-6">
            <div className="flex flex-col gap-1 mb-6">
              <h3 className="text-xl font-bold">
                🎯 {localize("Personalized Recommendations & Feature Drivers", "व्यक्तिगत सिफारिशें और मुख्य कारक")}
              </h3>
              <p className="text-xs text-[var(--muted)]">
                {localize(
                  "Feature-specific biometric explanations generated by Framingham SHAP attribution and clinical protocols.",
                  "फ्रामिंघम SHAP एट्रिब्यूशन और क्लिनिकल प्रोटोकॉल द्वारा उत्पन्न बायोमेट्रिक विश्लेषण।"
                )}
              </p>
            </div>

            {/* SHAP XAI Top Contributing Factors */}
            {((result.topContributingFactors && result.topContributingFactors.length > 0) ||
              (result.keyContributingFactors && result.keyContributingFactors.length > 0)) && (
              <div className="mb-6 space-y-3">
                <div className="flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-cyan-400">
                  <span>🧠</span>
                  <span>{localize("Biometric Factors Driving Your Score (SHAP Attribution)", "आपके स्कोर को प्रभावित करने वाले बायोमेट्रिक कारक (SHAP विश्लेषण)")}</span>
                </div>

                <div className="grid gap-3 sm:grid-cols-2">
                  {(result.topContributingFactors && result.topContributingFactors.length > 0
                    ? result.topContributingFactors.slice(0, 6)
                    : (result.keyContributingFactors || []).slice(0, 6)
                  ).map((factor: any, fIdx: number) => {
                    const isRiskElevating =
                      factor.direction === "higher" ||
                      factor.effect === "higher" ||
                      (typeof factor.impact === "number" && factor.impact > 0);
                    const impactVal = factor.impact !== undefined ? Number(factor.impact).toFixed(2) : "";
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center justify-between gap-2">
                          <div className="flex items-center gap-2">
                            <span className="text-base">{isRiskElevating ? "⚠️" : "🛡️"}</span>
                          ...
                          >
                            {isRiskElevating
                              ? `${impactVal ? `+${impactVal} ` : ""}${localize("Elevates Risk", "जोखिम बढ़ाता है")}`
                              : `${impactVal ? `${impactVal} ` : ""}${localize("Protective", "सुरक्षात्मक")}`}
                          </span>
                        </div>
                        <p className="mt-2 text-xs leading-relaxed text-[var(--muted)]">
                          {factor.explanation}
                          ...
                          {reason && (
                            <p className="mt-2 text-xs italic text-[var(--muted)]">
                              <span className="font-semibold not-italic text-cyan-400/90">
                                {localize("Clinical Basis:", "क्लिनिकल आधार:")}{" "}
                              </span>
                              {reason}
                            </p>
                          )}
